...
A presence-absence matrix is only as good as the records behind it, so downstream analysis needs occurrences whose names resolve to an accepted taxonomy, whose coordinates have been validated, and whose native status is known. The VasGBIF pipeline delivers that in four stages:

*Stage 1 — Import and issue parsing*

1.  **`import_records()`** — read the GBIF occurrence download ZIP (`SIMPLE_CSV` or Darwin Core Archive), keeping the raw `issue` field intact
2.  **`extract_gbif_issues()`** — expand `issue` into one logical column per GBIF issue code, plus a per-record issue count

*Stage 2 — Taxonomic resolution and record filtering*

3.  **`check_taxon()`** — resolve scientific names against WCVP through TNRS, keeping matches that clear the `accuracy` threshold and resolve below genus level
4.  **`custom_filter()`** — join the three outputs by `gbifID` and drop records rule by rule, logging every step

*Stage 3 — Coordinate validation and native-status annotation*

5.  **`refine_coordinates()`** — run CoordinateCleaner tests in parallel, splitting records into coordinate-clean, coordinate-problematic, and coordinate-less tables
6.  **`detect_native_status()`** — classify each record as `native`, `introduced`, `extinct`, `location_doubtful`, or `unknown`
