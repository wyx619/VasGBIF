...
5.  **Refine Coordinates** — **`refine_coordinates()`** runs CoordinateCleaner tests in parallel, splitting records into coordinate-clean, coordinate-problematic, and coordinate-less tables
6.  **Detect Native Status** — **`detect_native_status()`** classifies each record as `native`, `introduced`, `extinct`, `location_doubtful`, or `unknown`
...
Every object prints a summary, so `filtered$summary` shows how many records each rule removed, and `native_detected` reports the status breakdown — both worth inspecting before committing to a matrix.

Two objects carry forward. `refined_coordinates$CoordinateCleaned` holds the records that passed every coordinate test, and `native_detected` holds one row per record with its `native_status`. Because `detect_native_status()` also classifies coordinate-less records, joining the two tables on `gbifID` keeps only native records that actually have coordinates — exactly what a presence-absence matrix requires.
