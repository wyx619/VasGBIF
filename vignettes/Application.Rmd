...
2.  **Check Taxon Name** — **`check_taxon()`** resolves scientific names against WCVP through TNRS, keeping matches that clear the `accuracy` threshold and resolve below genus level
3.  **Customized Filter** — **`customized_filter()`** joins the three outputs by `gbifID` and drops records rule by rule, logging every step
4.  **Clean Coordinates** — **`par_clean_coordinates()`** runs CoordinateCleaner tests in parallel, splitting records into coordinate-clean and coordinate-problematic tables
5.  **Detect Native Status** — **`detect_native_coord()`** classify each record as `native`, `introduced`, `extinct`, `location_doubtful`, or `unknown`
6.  **Map Records** — **`map_records()`** renders the records on an interactive map with geohash-based decluttering
7.  **Export Records** — **`export_records()`** exports records to gzip-compressed CSV files
